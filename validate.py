...
class Validator:
    ...
    def validate_meta(self):
        ...
        self.check(
            ...
            (
                ...
                and self.is_iso_datetime(
                    meta.get("generated_utc")
                )
            )
        )
    ...
    def validate_scene(self):
        ...
        self.check(
            "scene.time",
            isinstance(scene.get("time"), str)
        )

        self.check(
            "scene.time_iso",
            self.is_iso_datetime(
                scene.get("time_iso")
            )
        )
        ...
        self.check(
            "scene.image",
            isinstance(scene.get("image"), str)
        )

        self.check(
            "scene.mask",
            isinstance(scene.get("mask"), str)
        )

        self.check(
            "scene.satellite",
            isinstance(scene.get("satellite"), str)
        )

        self.check(
            "scene.mode",
            isinstance(scene.get("mode"), str)
        )

        self.check(
            "scene.polarisation",
            isinstance(
                scene.get("polarisation"),
                str
            )
        )
    ...
    def validate_detection(self):
        ...
        self.check(
            ...
            self.is_score(
                detection.get("lookalike_prob")
            )
        )

        # -----------------------------------------------------
        # Slick information lives inside detection.slick
        # -----------------------------------------------------
        ...
        polygon_has_valid_count = (
            polygon_is_list
            and 20 <= len(polygon) <= 60
        )

        polygon_points_valid = (
            polygon_is_list
            and all(
                self.is_lon_lat(point)
        ...
        self.check(
            "detection.slick.polygon",
            polygon_valid
        )

        self.check(
            "detection.slick.head",
            self.is_lon_lat(
                slick.get("head")
            )
        )
        ...
        # age_hours_est may legitimately be null
    ...
    def validate_drift(self):
        ...
        # Contract explicitly allows drift to be null
        ...
        # Ensemble is optional and may be []
    ...
    def validate_vessels(self):
        ...
        for index, vessel in enumerate(vessels):
            ...
            # Check score fields individually
            ...
            # Reason rule
            #
            # cleared -> non-empty reason
            # accused/suspect -> reason must be null
            # -------------------------------------------------
            ...
            # - null
            # - list of [lon, lat]
            # - list containing null entries for AIS gaps
            # -------------------------------------------------
        ...
        # -----------------------------------------------------
        # NEW LANE A INVARIANT
        #
        # A vessel may only be "accused" if:
        # 1. It is ranked first by score
        # 2. Its score beats the second-place vessel by >= 0.08
        #
        # This catches silent attribution-rule violations.
        # -----------------------------------------------------
        ...
        if accused_vessels:
            ...
            if len(valid_vessel_dicts) >= 2:
                ...
                ranked_vessels = sorted(
                    valid_vessel_dicts,
                    key=lambda vessel: vessel.get("score"),
                    reverse=True
                )
            ...
            # No accused vessel is perfectly valid.
            # The margin rule only applies when an accusation exists.
    ...
    def validate_summary(self):
        ...
        # Calculate actual counts from vessels
        ...
        suspect_vessels = [
            ...
            if (
                isinstance(vessel, dict)
                and vessel.get("verdict") == "suspect"
            )
        ]
        ...
        cleared_vessels = [
            ...
            if (
                isinstance(vessel, dict)
                and vessel.get("verdict") == "cleared"
            )
        ]
        ...
        #
        # If no_attribution:
        #     accused_mmsi MUST be null
        #
        # If accused:
        #     accused_mmsi MUST exist
        #     and correspond to a vessel
        # -----------------------------------------------------

        accused_mmsi = summary.get(
            "accused_mmsi"
        )
        ...
        self.check(
            "summary.n_scored",
            summary.get("n_scored") == len(vessels)
        )
        ...
        # n_accused is not required by the frozen contract,
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print(
            "Usage: python validate.py <data.json>"
        )
    ...
    if not json_path.exists():
        ...
        print(
            f"ERROR: File not found: {json_path}"
        )
